...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      {/* ── App Header ── */}
      <header className="fixed top-0 inset-x-0 h-14 lg:h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 z-40 flex items-center justify-between px-4 lg:px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span className="w-8 h-8 rounded-full bg-brand-800 text-white flex items-center justify-center text-xs font-medium">
              {initials}
            </span>
          </button>
          <button
            onClick={handleLogout}
            className="btn-ghost btn-sm text-slate-500 hover:text-red-600 dark:text-slate-400"
          >
            Logout
          </button>
        </div>
      ...
      {/* ── Desktop Sidebar ── */}
      <aside className="fixed left-0 top-16 bottom-0 w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 p-4 hidden lg:flex lg:flex-col z-30">
      ...
      {/* ── Main Content ── */}
      <main className="pt-14 lg:pt-16 pb-16 lg:pb-4 lg:pl-64 px-4 lg:px-8">
        <Outlet />
      </main>

      {/* ── Mobile Bottom Nav ── */}
      <nav className="fixed bottom-0 inset-x-0 h-14 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 z-40 flex items-center justify-around lg:hidden pb-safe">
