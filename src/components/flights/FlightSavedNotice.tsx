...
import { SaveWarningList } from '../saveWarnings/SaveWarnings';
import { useInlineSaveWarnings } from '../saveWarnings/useInlineSaveWarnings';
import { useSaveWarningsStore } from '../../stores/saveWarningsStore';
 ...
 * and the save's warnings; stays until dismissed while it shows warnings.
 */
export function FlightSavedNotice({ saved, onLogAnother, onDismiss, timeoutMs = 12_000 }: FlightSavedNoticeProps) {
  const { t } = useTranslation('flights');
  const warnings = useInlineSaveWarnings();
  const clearWarnings = useSaveWarningsStore((s) => s.clear);
  ...
  }, [saved, onDismiss, timeoutMs, hasWarnings]);
  ...
  return (
    ...
    >
      ...
      <SaveWarningList entries={warnings} className="mt-2" />
      {flight && onLogAnother && (
