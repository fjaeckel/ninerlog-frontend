...
export default function FlightForm({ flightId, onClose }: FlightFormProps) {
  ...
  useEffect(() => {
    if (existingFlight && isEditing) {
      ...
      if (existingFlight.crewMembers) {
        setCrewMembers(
          existingFlight.crewMembers.map((m: { contactId?: string | null; name: string; role: string }) => ({
      ...
      if (existingFlight.approaches && existingFlight.approaches.length > 0) {
        setApproaches(
          existingFlight.approaches.map((a: { type: string; airport?: string | null; runway?: string | null }) => ({
            type: a.type,
            airport: a.airport || '',
            runway: a.runway || '',
          }))
  ...
  const matchedAircraft = (aircraftList ?? []).find((ac) => ac.registration.toUpperCase() === regUppercase);
  ...
  const showLaunchMethod =
    currentAircraftClass === 'TMG' ||
    currentAircraftClass === 'GLIDER' ||
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-full overflow-x-hidden">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-blue-800 dark:text-blue-300">
              {t('form.fillFromLastFlightTitle')}
            </p>
            <p className="text-xs text-blue-600 dark:text-blue-400 truncate">
              {lastFlight.aircraftReg} · {lastFlight.departureIcao || '?'} → {lastFlight.arrivalIcao || '?'}
            </p>
          </div>
          <button type="button" onClick={handleAutoFill} className="btn-primary btn-sm text-xs shrink-0">
            {t('form.fill')}
          </button>
        </div>
      )}
      ...
      <fieldset disabled={isLocked} className="space-y-6 border-0 p-0 m-0 min-w-0">
        {/* Basic Info */}
        <fieldset>
          <legend className="text-sm font-semibold text-slate-800 dark:text-slate-100 mb-3">
            {t('sections.basic')}
          </legend>
          <div className="grid grid-cols-2 gap-4 [&>*]:min-w-0">
            ...
            <div className="relative" ref={suggestionsRef}>
              ...
              />
              {errors.aircraftReg && <p className="form-error">{errors.aircraftReg.message}</p>}
        ...
        <fieldset>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4 [&>*]:min-w-0">
            <div>
              ...
              <input
                ...
                maxLength={100}
              />
              {errors.departureIcao && <p className="form-error">{errors.departureIcao.message}</p>}
            </div>
            <div>
              ...
              <input
                ...
                maxLength={100}
              />
              {errors.arrivalIcao && <p className="form-error">{errors.arrivalIcao.message}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4 [&>*]:min-w-0">
            <div>
              ...
              />
              {errors.offBlockTime && <p className="form-error">{errors.offBlockTime.message}</p>}
            </div>
            <div>
              ...
              />
              {errors.onBlockTime && <p className="form-error">{errors.onBlockTime.message}</p>}
            </div>
            <div>
        ...
        <fieldset>
          ...
          <div className="space-y-3">
            ...
            {crewMembers.map((member, idx) => (
              <div
                key={idx}
                className="flex items-center gap-2 text-sm bg-slate-50 dark:bg-slate-700/50 rounded-lg p-2"
              >
              ...
              <button
                ...
                onClick={() => {
                  if (crewNameInput.trim()) {
                    setCrewMembers((prev) => [
                      ...prev,
                      { name: crewNameInput.trim(), role: crewRoleInput, contactId: null },
                    ]);
            ...
            {crewMembers.some((m) => m.role === 'Instructor') && (
              <div className="mt-3">
                <label htmlFor="instructorComments" className="form-label">
                  {t('fields.instructorComments', { defaultValue: 'Instructor Comments' })}
                </label>
                <input
                  {...register('instructorComments')}
                  id="instructorComments"
                  className="input text-sm"
                  placeholder={t('form.instructorRemarks')}
                />
              </div>
            )}
          </div>
        ...
        {isEditing && existingFlight && (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div>
              <label className="form-label">{t('detail.totalBlockTime')}</label>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <div className="input bg-slate-50 dark:bg-slate-700 text-slate-700 dark:text-slate-200">
                  {existingFlight.isPic
                    ? 'PIC'
                    : existingFlight.isDual
                      ? 'Dual'
                      : (existingFlight.sicTime || 0) > 0
                        ? 'SIC'
                        : '—'}
                </div>
          ...
          {expandedSections.instrument && (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div>
                  <label htmlFor="actualInstrumentTime" className="form-label">
                    {t('fields.actualInstrumentTime')}
                  </label>
                  <input
                ...
                <div>
                  <label htmlFor="simulatedInstrumentTime" className="form-label">
                    {t('fields.simulatedInstrumentTime')}
                  </label>
                  <input
                ...
                <div>
                  <label htmlFor="holds" className="form-label">
                    {t('fields.holds')}
                  </label>
                  <input
                ...
                {approaches.length > 0 && (
                  <div className="space-y-2">
                    <div className="sr-only sm:not-sr-only sm:grid sm:grid-cols-[2fr_1fr_1fr_auto] gap-2 text-xs text-slate-500 dark:text-slate-400 font-medium px-0.5">
                      <span>{t('approachType')}</span>
                      <span>{t('approachAirport')}</span>
                      <span>{t('approachRunway')}</span>
                      <span></span>
                    </div>
                    {approaches.map((appr, idx) => (
                      <div key={idx} className="grid grid-cols-[2fr_1fr_1fr_auto] gap-1.5 items-center">
                        <select
                          ...
                          onChange={(e) => {
                            const a = [...approaches];
                            a[idx] = { ...a[idx], type: e.target.value };
                            setApproaches(a);
                          }}
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const a = [...approaches];
                            a[idx] = { ...a[idx], airport: e.target.value.toUpperCase() };
                            setApproaches(a);
                          }}
                          className="input text-sm py-1.5"
                          placeholder="ICAO"
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const a = [...approaches];
                            a[idx] = { ...a[idx], runway: e.target.value };
                            setApproaches(a);
                          }}
                          className="input text-sm py-1.5"
                          placeholder="Rwy"
                        ...
                        <button
                          ...
                          className="text-red-400 hover:text-red-600 dark:text-red-500 dark:hover:text-red-400 p-1 rounded hover:bg-red-50 dark:hover:bg-red-900/20"
                        >
                          <X size={14} />
                        </button>
                      </div>
        ...
        <fieldset>
          ...
          {expandedSections.training && (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div>
                  <label htmlFor="groundTrainingTime" className="form-label">
                    {t('fields.groundTrainingTime')}
                  </label>
                  <input
                ...
                <div>
                  <label htmlFor="simulatedFlightTime" className="form-label">
                    {t('fields.simulatedFlightTime')}
                  </label>
                  <input
                ...
                <div>
                  <label htmlFor="multiPilotTime" className="form-label">
                    {t('fields.multiPilotTime')}
                  </label>
                  <input
              ...
              {(watch('simulatedFlightTime') > 0 || watch('fstdType')) && (
                <div className="mt-3">
                  <label htmlFor="fstdType" className="form-label">
                    {t('fields.fstdType')}
                  </label>
                  <input
                    {...register('fstdType')}
