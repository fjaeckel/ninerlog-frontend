        ...
        return (
          <>
            <div className="sm:hidden mb-6">
              <select value={tab} onChange={(e) => setTab(e.target.value as Tab)} className="input w-full">
                {tabs.map(({ id, label }) => (
...
function DashboardTab() {
  ...
  const backupProviders = backups ? Object.entries(backups.byProvider).sort(([a], [b]) => a.localeCompare(b)) : [];
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {stats.map((s) => (
          <div key={s.label} className="card text-center py-4">
            <div
              className={`text-2xl font-bold ${s.warn ? 'text-amber-600 dark:text-amber-400' : 'text-slate-800 dark:text-slate-100'}`}
            >
              {s.value}
            </div>
          ...
          {backupProviders.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {backupProviders.map(([provider, count]) => (
                <div
                  key={provider}
                  className="rounded border border-slate-200 dark:border-slate-700 px-3 py-2 text-center"
                >
...
function UsersTab() {
  ...
  const handleAction = async () => {
    ...
    if (
      ...
      deleteEmailInput.trim().toLowerCase() !== confirmAction.user.email.toLowerCase()
    ) {
      return;
    }
    setActionMessage('');
    try {
      ...
      setActionMessage(`Failed to ${confirmAction.type} user.`);
    }
  ...
  return (
    <>
      {actionMessage && (
        <div
          className={`mb-4 px-4 py-3 rounded-lg text-sm ${actionMessage.includes('Failed') ? 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400' : 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400'}`}
        >
          {actionMessage}
        </div>
      )}
      ...
      <div className="card overflow-hidden">
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col items-start gap-1">
                      {u.disabled ? (
                        ...
                        <span className="badge badge-current text-xs">{t('admin.users.active')}</span>
                      )}
                      {u.emailVerified ? (
                        ...
                        <span
                          ...
                          title={t('admin.users.unverifiedTitle')}
                        >
                  ...
                  <td className="px-4 py-3">
                    {u.twoFactorEnabled ? (
                      ...
                      <span className="text-slate-400 text-xs">{t('admin.users.off')}</span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1 flex-wrap">
                      {u.disabled ? (
                        <button
                          onClick={() => setConfirmAction({ type: 'enable', user: u })}
                          className="btn-ghost btn-sm text-xs text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20"
                        ...
                        <button
                          onClick={() => setConfirmAction({ type: 'disable', user: u })}
                          className="btn-ghost btn-sm text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"
                      ...
                      {u.locked && (
                        <button
                          onClick={() => setConfirmAction({ type: 'unlock', user: u })}
                          className="btn-ghost btn-sm text-xs text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20"
                      ...
                      {u.twoFactorEnabled && (
                        <button
                          onClick={() => setConfirmAction({ type: 'reset-2fa', user: u })}
                          className="btn-ghost btn-sm text-xs text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20"
                          title="Reset 2FA"
                        >
                      ...
                      <button
                        ...
                        className="btn-ghost btn-sm text-xs text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20"
        ...
        <div className="sm:hidden divide-y divide-slate-100 dark:divide-slate-800">
          ...
          {data?.data?.map((u) => (
            <div key={u.id} className="px-4 py-3 space-y-2">
              ...
              <div className="flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400">
                <span>
                  {u.flightCount} {t('admin.users.flights')}
                </span>
                <span>
                  {u.aircraftCount} {t('admin.users.aircraft')}
                </span>
          ...
          {!isLoading && data?.data?.length === 0 && (
            <div className="px-4 py-8 text-center text-slate-400">{t('admin.users.noUsers')}</div>
          )}
        </div>
      ...
      {confirmAction && (
        <>
          <div
            className="fixed inset-0 bg-black/40 z-[1020]"
            onClick={() => {
              setConfirmAction(null);
              setDeleteEmailInput('');
            }}
          ...
          <div className="fixed inset-x-4 top-1/2 -translate-y-1/2 z-[1020] mx-auto max-w-md bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-6">
            <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">
              {t('admin.users.confirmAction', { action: actionLabel(confirmAction.type) })}
            </h3>
            <p className="text-sm text-slate-600 dark:text-slate-400 mb-1">
              ...
              {confirmAction.type === 'reset-2fa' &&
                t('admin.users.reset2faConfirm', { email: confirmAction.user.email })}
              ...
              <>
                <p className="text-xs text-red-600 dark:text-red-400 mb-3 font-medium">
                  {t('admin.users.deleteWarning', {
                    ...
                    flightCount: confirmAction.user.flightCount,
                    aircraftCount: confirmAction.user.aircraftCount,
                  })}
                </p>
                <label
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                ...
                className={
                  confirmAction.type === 'disable' || confirmAction.type === 'delete' ? 'btn-danger' : 'btn-primary'
                }
              >
                {actionLabel(confirmAction.type)}
              </button>
            </div>
          </div>
        </>
...
function AuditLogTab() {
  ...
  const renderUser = (id: string | undefined, name: string | undefined, email: string | undefined) => {
    if (!id) return '\u2014';
    if (name && email) {
      return (
        <span>
  ...
  return (
    <div className="card overflow-hidden">
            ...
            {data?.data?.map((entry) => (
              <tr key={entry.id} className="border-b border-slate-100 dark:border-slate-800">
                ...
                <td className="px-4 py-3 text-xs">
                  {renderUser(entry.adminUserId, entry.adminName, entry.adminEmail)}
                </td>
                <td className="px-4 py-3 text-xs">
                  {renderUser(entry.targetUserId, entry.targetUserName, entry.targetUserEmail)}
                </td>
              </tr>
            ))}
            {!isLoading && data?.data?.length === 0 && (
      ...
      <div className="sm:hidden divide-y divide-slate-100 dark:divide-slate-800">
        ...
        {data?.data?.map((entry) => (
          <div key={entry.id} className="px-4 py-3 space-y-1">
            ...
            <div className="flex flex-col gap-1 text-xs text-slate-400">
              <span>
                {t('admin.audit.admin')}: {renderUser(entry.adminUserId, entry.adminName, entry.adminEmail)}
              </span>
              {entry.targetUserId && (
                <span>
                  {t('admin.audit.targetUser')}:{' '}
                  {renderUser(entry.targetUserId, entry.targetUserName, entry.targetUserEmail)}
                </span>
              )}
            </div>
        ...
        {!isLoading && data?.data?.length === 0 && (
          <div className="px-4 py-8 text-center text-slate-400">{t('admin.audit.noEntries')}</div>
        )}
      </div>
      {data && data.pagination.totalPages > 1 && (
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-slate-700">
          ...
          <div className="flex gap-2">
            <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="btn-secondary btn-sm text-xs">
              {t('admin.users.previous')}
            </button>
            <button
              disabled={page >= data.pagination.totalPages}
...
function MaintenanceTab() {
  ...
  return (
    <div className="space-y-6">
      {message && (
        <div
          className={`px-4 py-3 rounded-lg text-sm ${message.includes('Failed') ? 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400' : 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400'}`}
        >
          {message}
        </div>
      )}
      <div className="card">
        <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">
          {t('admin.maintenance.tokenCleanup')}
        </h3>
...
function ConfigTab() {
  ...
  const rows: { label: string; value: React.ReactNode }[] = [
    ...
    },
    {
      ...
      value:
        data.cloudBackupProviders.length > 0 ? (
...
function AnnouncementsTab() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFeedback('Failed to create announcement.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      {feedback && (
        <div
          className={`px-4 py-3 rounded-lg text-sm ${feedback.includes('Failed') ? 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400' : 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400'}`}
        >
          {feedback}
        </div>
      )}
      <div className="card">
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          <div>
            <label className="form-label" htmlFor="ann-message">
              {t('admin.announcements.message')}
            </label>
            <textarea
              id="ann-message"
              value={message}
          ...
          <div className="flex gap-4 flex-wrap">
            <div className="flex-1 min-w-[200px]">
              <label className="form-label" htmlFor="ann-severity">
                {t('admin.announcements.severity')}
              </label>
              <select
                id="ann-severity"
                value={severity}
            ...
            <div className="flex-1 min-w-[200px]">
              <label className="form-label" htmlFor="ann-expires">
                {t('admin.announcements.expireAfter')}
              </label>
              <input
                id="ann-expires"
                type="number"
                ...
                className="input mt-1"
                placeholder={t('admin.announcements.expirePlaceholder')}
              />
            </div>
          </div>
      ...
      <div className="card">
        <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">
          {t('admin.announcements.activeTitle')}
        </h3>
        {isLoading && <p className="text-slate-400 text-sm">{t('common:loading')}</p>}
        {data?.announcements?.length === 0 && !isLoading && (
          <p className="text-slate-400 text-sm">{t('admin.announcements.noAnnouncements')}</p>
        )}
          ...
          {data?.announcements?.map((a) => {
            ...
            return (
              ...
              >
                ...
                {a.expiresAt && (
                  <span className="text-xs text-slate-400">
                    {t('admin.announcements.expires')} {fmtDateTime(a.expiresAt)}
                  </span>
                )}
                <button
