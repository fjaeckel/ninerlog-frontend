    ...
    <div className="mx-auto max-w-7xl py-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 id="aircraft-form-title" className="text-xl font-semibold text-slate-800 dark:text-slate-100">
      ...
      {!aircraft || aircraft.length === 0 ? (
        <div className="card text-center py-12">
          <p className="text-slate-500 dark:text-slate-400 mb-4">{t('noAircraft')}</p>
                ...
                <div className="flex flex-col lg:flex-row lg:items-start gap-4 lg:gap-6">
                  ...
                  <dl className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-4 gap-x-6 gap-y-3 flex-1 text-sm min-w-0">
                    <div>
                      ...
                      <dd className="mt-0.5 font-medium text-slate-700 dark:text-slate-200">{ac.type}</dd>
                    </div>
                    {ac.aircraftClass && (
                      <div>
                        <dt className="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">
                    ...
                    <div className="col-span-2 sm:col-span-1 lg:col-span-2 xl:col-span-2">
                      ...
                      <dd className="mt-1 flex flex-wrap gap-1.5">
                        {ac.isComplex && <span className="badge-info">{t('fields.isComplex')}</span>}
                        {ac.isHighPerformance && <span className="badge-expiring">{t('highPerf')}</span>}
                        {ac.isTailwheel && <span className="badge-neutral">{t('fields.isTailwheel')}</span>}
                    ...
                    <div className="col-span-2 sm:col-span-3 lg:col-span-2 xl:col-span-4 pt-3 mt-1 border-t border-slate-100 dark:border-slate-700">
                      {stats ? (
                        ...
                        <p className="text-xs text-slate-400 dark:text-slate-500">{t('stats.neverFlown')}</p>
                      )}
                      ...
                      {recencyPrefs.perModel && typeStats && (
                        <p className="mt-2 text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                          ...
                          {typeStats.lastFlightDate && (
                            <span>· {t('stats.modelLastFlown', { date: fmtDate(typeStats.lastFlightDate) })}</span>
                          )}
                        </p>
                      )}
                    </div>
                    {ac.notes && (
                      <div className="col-span-2 sm:col-span-3 lg:col-span-2 xl:col-span-4">
                        ...
                        <dd className="mt-0.5 text-slate-600 dark:text-slate-300 italic break-words">{ac.notes}</dd>
                      </div>
                    )}
                  </dl>
