...
import {
  ...
  useUpdateCustomCurrency,
  useDeleteCustomCurrency,
  usePreviewCustomCurrency,
...
import type { CustomCurrencyEvaluation, CustomRuleInput, CustomCurrencyRule } from '../../types/customCurrency';
...
function validateDraft(input: CustomRuleInput): string | null {
  ...
  if (!input.definition.requirements || input.definition.requirements.length === 0)
...
export default function CustomCurrencyBuilderPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-4">
      ...
      {shareParam && (
        <div
          className="card border-l-4 border-l-sky-500 bg-sky-50/50 dark:bg-sky-900/15"
          data-testid="share-import-banner"
        >
          ...
          <div className="space-y-2">
            ...
            {rules && rules.length === 0 && (
              <p className="text-sm text-slate-500 dark:text-slate-400">
                No custom rules yet. Create one, or import a shared rule below.
              </p>
            )}
            {rules?.map((item) => (
              <button
              ...
              >
                ...
                <span
                  className={`ml-2 inline-block w-2 h-2 rounded-full align-middle ${
                    item.evaluation.status === 'current'
                      ? 'bg-green-500'
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() =>
                    canSave && current.input && preview.mutate(current.input.definition, { onSuccess: setEvaluation })
                  }
                  disabled={!canSave}
              ...
              {shareLink && currentRule?.rule.isShared && (
                <div
                  className="rounded-lg border border-slate-200 dark:border-slate-700 p-2 flex items-center gap-2"
              ...
              {current.input ? (
                evaluation ? (
                  <CustomCurrencyCard
                    item={{
                      rule: {
                        ...
                        emoji: current.input.emoji,
                        definition: current.input.definition,
                        enabled: true,
                        notify: false,
                        isShared: false,
                        createdAt: '',
                  ...
                  <p className="text-xs text-slate-400">
                    {preview.isPending ? 'Evaluating against your flights…' : 'Preview updates as you edit.'}
                  </p>
                )
              ) : (
