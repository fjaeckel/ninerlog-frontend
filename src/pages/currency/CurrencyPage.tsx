...
import {
  ...
  useSetEnabledCustomCurrency,
  useSetNotifyCustomCurrency,
...
export default function CurrencyPage() {
  ...
  const expiringRatings =
    currencyStatus?.ratings.filter((r) => r.status === 'expiring' || r.status === 'expired') || [];
  const now = new Date();
  ...
  return (
    <div className="mx-auto max-w-[960px] py-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {totalAlerts > 0 && <span className="badge-expiring">{t('alerts', { count: totalAlerts })}</span>}
      </div>

      {isLoading && <div className="text-center py-12 text-slate-400 dark:text-slate-500">{t('loading')}</div>}
            ...
            >
              ...
              <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">
                {t('customCurrency.emptyHint', {
                  defaultValue:
                    'Combine flights, aircraft and requirements into rules we track for you — and share them.',
                })}
              </p>
            </Link>
          )}
        ...
        <div className="mb-8">
          ...
          {currencyStatus.flightReview && (
            <div
              className={`card border-l-4 mb-4 ${
                currencyStatus.flightReview.status === 'current'
                  ? 'border-l-green-500 bg-green-50 dark:bg-green-900/20'
                  : currencyStatus.flightReview.status === 'expiring'
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-semibold text-sm text-slate-800 dark:text-slate-100">✈️ {t('flightReview')}</h3>
                <span
                  className={
                    currencyStatus.flightReview.status === 'current'
                      ? 'badge-current'
          ...
          {Object.keys(ratingsByLicense).length === 0 && (
            <div className="card text-center py-8">
              <p className="text-slate-500 dark:text-slate-400 text-sm">{t('noRatings')}</p>
            </div>
          )}
                ...
                >
                  ...
                  <span className="font-semibold text-slate-800 dark:text-slate-100 flex-1">
                    {license
                      ...
                      : ratings[0]?.regulatoryAuthority || 'License'}
                        ...
                        {rating.ruleDescription && (
                          <p className="text-xs text-slate-400 dark:text-slate-500 mt-1 px-1 italic">
                            {(rating as any).ruleDescriptionKey
                              ? t(`ruleDescriptions.${(rating as any).ruleDescriptionKey}`, {
                                  defaultValue: rating.ruleDescription,
                                })
                              : rating.ruleDescription}
                          </p>
                        )}
      ...
      {!isLoading &&
        currencyStatus &&
        currencyStatus.passengerCurrency &&
        currencyStatus.passengerCurrency.length > 0 && (
          <div className="mb-8" data-testid="passenger-currency-section">
            <h2 className="section-title mb-4">{t('passengerCurrency')}</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">{t('passengerCurrencyDesc')}</p>
            <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  >
                    ...
                    <div className="space-y-1 mb-1">
                      <div className="flex justify-between items-center text-xs">
                        <span className="font-medium text-slate-700 dark:text-slate-300">{dayOk ? '✓' : '○'} Day</span>
                    ...
                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-2 italic">
                      {(pax as any).ruleDescriptionKey
                        ? t(`ruleDescriptions.${(pax as any).ruleDescriptionKey}`, {
                            defaultValue: pax.ruleDescription,
                          })
                        : pax.ruleDescription}
                    </p>
                  </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {credentials.map((cred) => {
              ...
              const description =
                CREDENTIAL_DESCRIPTIONS[cred.credentialType] || cred.credentialType.replace(/_/g, ' ');
                ...
                <div key={cred.id} className={`card border-l-4 ${statusColor}`} data-testid={`credential-${cred.id}`}>
                  ...
                  <div className="text-xs text-slate-600 dark:text-slate-400 space-y-1">
                    {cred.issuingAuthority && (
                      <p>
                        {t('issuingAuthority')}: {cred.issuingAuthority}
                      </p>
                    )}
                    <p>
                      {t('issued')}: {cred.issueDate}
                    </p>
                    {cred.expiryDate && (
                      <p>
                        ...
                        {daysLeft !== null && (
                          <span
                            className={`ml-1 font-medium font-mono tabular-nums ${expired ? 'text-red-600 dark:text-red-400' : expiringSoon ? 'text-amber-600 dark:text-amber-400' : 'text-green-600 dark:text-green-400'}`}
                          >
                            (
                            {expired
                              ? t('expiredAgo', { days: Math.abs(daysLeft) })
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-2 italic">{description}</p>
                </div>
              );
            })}
          </div>
      ...
      {!isLoading && (!credentials || credentials.length === 0) && (
        <div className="card text-center py-8 mb-8">
          <p className="text-slate-500 dark:text-slate-400 text-sm">{t('noCredentials')}</p>
        </div>
      )}
