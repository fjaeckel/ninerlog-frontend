...
import { createTestUser, login, seedLicense, seedClassRating, seedCredential, type AuthContext } from './helpers';
...
test.describe('Currency Page', () => {
  ...
  test('should display currency page', async ({ page }) => {
    ...
    await expect(page.getByText('Currency & Compliance')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should show currency card for a class rating', async ({ page }) => {
    ...
    await page.getByRole('link', { name: 'Currency' }).first().click();
    await expect(page.getByRole('heading', { name: /flight currency/i })).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should show credentials section', async ({ page }) => {
    ...
    await page.getByRole('link', { name: 'Currency' }).first().click();
    await page.reload();
    await expect(page.getByText('Credentials & Medicals')).toBeVisible({ timeout: 10000 });
  });
});
